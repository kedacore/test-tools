...
func addTaskInstances(numberOfTaskInstancesToAdd int) {
	...
	for i := 0; i < numberOfTaskInstancesToAdd; i++ {
		...
		insert, err := db.Query("INSERT INTO task_instance (state) VALUES (?)", queued)
		insert.Close()
		if err != nil {
			panic(err.Error())
		}
...
func updateTaskInstances(numberOfTaskInstancesToUpdate int) {
	...
	for i := 0; i < numberOfTaskInstancesToUpdate; i++ {
		...
		err = tx.QueryRowContext(ctx, "SELECT id, state FROM task_instance WHERE state = ? LIMIT 1", queued).Scan(&taskInstance.id, &taskInstance.state)
		if err != nil {
			tx.Rollback()
			log.Fatal(err)
		...
		updateStmt, err := tx.PrepareContext(ctx, "UPDATE task_instance SET state = ? WHERE id = ?")
		if err != nil {
			tx.Rollback()
